...
>(({ className, ...props }, ref) => (
	<ToastPrimitives.Viewport
		ref={ref}
		className={cn(
			"fixed left-1/2 top-0 z-[100] flex max-h-screen min-w-max -translate-x-1/2 flex-col-reverse px-2 py-1.5 sm:right-2 sm:top-3 sm:max-w-sm sm:flex-col-reverse",
			className
		)}
		{...props}
	/>
...
const toastVariants = cva(
	"data-[swipe=move]:transition-none dark:border-neutral-700/60 bg-white dark:bg-[#1c1c1c] border border-neutral-200/80 group relative pointer-events-auto flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg py-2.5 px-3.5 shadow-md dark:shadow-black/20 transition-all data-[swipe=move]:transition-x-[var(--radix-toast-swipe-move-x)] data-[swipe=cancel]:transition-x-0 data-[swipe=end]:transition-x-[var(--radix-toast-swipe-end-x)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=open]:slide-in-from-top-full data-[state=closed]:slide-out-to-top-full",
	{
		variants: {
			variant: {
				error: "",
...
>(({ className, ...props }, ref) => (
	<ToastPrimitives.Close
		ref={ref}
		className={cn(
			"z-50 self-end rounded-md p-1 focus:outline-none focus:ring-2 focus:ring-transparent group-hover:opacity-100",
			className
		)}
		toast-close=""
		{...props}
